...
# Redis 分库配置
from openai_chat.settings.base import REDIS_DB_USERS_REGISTER_CACHE # 用户预注册缓存分库
...
class RegisterPreService:
    ...
    def process(self) -> Dict[str, Any]:
        ...
        try:
            ...
            cooldown_ok = self.redis.set(
				cooldown_key,
                "1",
                ex=self.COOLDOWN_TTL_SECONDS,
                nx=True,
			)
            cost_ms = int((time.time() - start) * 1000)
            logger.info("[用户注册] 冷却窗口写入耗时: %sms", cost_ms)
        ...
        try:
            start = time.time()
            self.redis.set(prereg_key, prereg_value, ex=self.PREREG_TTL_SECONDS)
            cost_ms = int((time.time() - start) * 1000)
            logger.info("[用户注册] 预注册信息写入耗时: %sms", cost_ms)
        except Exception as e:
        ...
        self._send_verify_email_async(verify_code)
    ...
    def _send_verify_email_async(self, verify_code: str) -> None:
        """
        调用 Celery 任务发送验证码邮件
        - delay/apply_async
        ...
        try:
            ...
            logger.exception("[用户注册] 发送验证码任务调度失败", extra={"email": self.email})
